...
bool IsVisible(MeshletData m, float4x4 world, float scale)
{
  ...
  float3 view = normalize(g_FrameConstants.CameraWS - apex);
...
void main(uint gtid : SV_GroupThreadID, uint dtid : SV_DispatchThreadID, uint gid : SV_GroupID)
{
  ...
  if (dtid < mi.numMeshlets) {
    visible = IsVisible(m, mi.worldMatrix, mi.scale);
  }
