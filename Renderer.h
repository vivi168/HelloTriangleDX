...
public:
  ...
  void SetSceneCamera(Camera* cam) { m_Scene.camera = cam; }
...
private:
  ...
  {
    // if the current m_Fences value is still less than "m_FenceValues", then we
    // know the GPU has not finished executing the command queue since it has
    // not reached the "m_CommandQueue->Signal(m_Fences, m_FenceValues)" command
    if (m_Fences[frameIndex]->GetCompletedValue() < m_FenceValues[frameIndex]) {
      // we have the m_Fences create an event which is signaled once the
      // m_Fences's current value is "m_FenceValues"
      CHECK_HR(m_Fences[frameIndex]->SetEventOnCompletion(
          m_FenceValues[frameIndex], m_FenceEvent));

      // We will wait until the m_Fences has triggered the event that it's
      // current value has reached "m_FenceValues". once it's value has reached
      // "m_FenceValues", we know the command queue has finished executing
  ...
  void WaitGPUIdle(size_t frameIndex)
  {
    ...
    CHECK_HR(m_CommandQueue->Signal(m_Fences[frameIndex].Get(),
                                    m_FenceValues[frameIndex]));
  ...
  ComPtr<ID3D12Fence> m_Fences[FRAME_BUFFER_COUNT];
  // a handle to an event when our m_Fences is unlocked by the gpu
  HANDLE m_FenceEvent;
  // this value is incremented each frame. each m_Fences will have its own value
  ...
  // size of the rtv descriptor on the m_Device (all front and back buffers will
  // be the same size)
  ...
  const size_t ConstantBufferPerObjectAlignedSize =
      AlignUp<size_t>(sizeof(PerObjectCB1_VS), 256);
