    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        D3D12_GPU_DESCRIPTOR_HANDLE cbvSrvHeapStart =
            bd->pd3dSrvDescHeap[bd->frameIndex]
                ->GetGPUDescriptorHandleForHeapStart();
        const UINT cbvSrvDescriptorSize =
            bd->pd3dDevice->GetDescriptorHandleIncrementSize(
                D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        CD3DX12_GPU_DESCRIPTOR_HANDLE cbvSrvHandle(cbvSrvHeapStart,
  ...
  // Upload texture to graphics system
  {
    ...
    for (size_t i = 0; i < bd->numFramesInFlight; ++i) {
      CD3DX12_CPU_DESCRIPTOR_HANDLE descHandle(
          bd->pd3dSrvDescHeap[i]->GetCPUDescriptorHandleForHeapStart(),
          cbvSrvDescriptorSize);
