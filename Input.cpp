...
void OnKeyUp(WPARAM key) { SetNewKeyState(key, false); }

void OnKeyDown(WPARAM key) { SetNewKeyState(key, true); }

bool IsHeld(KB key) { return inputState.keyState[key].held; }

bool IsPressed(KB key) { return inputState.keyState[key].pressed; }

bool IsReleased(KB key) { return inputState.keyState[key].released; }
}  // namespace Input
